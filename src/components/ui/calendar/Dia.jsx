    ...
    return (
        <div onClick={onClick} className={`calendar__day ${day?.esHoy ? 'calendar__day-active' : ''}`}>
            ...
            <div className="calendar__day-group">
                {day.citas?.length > 0 && 
                    day.citas.map( ({estado}, i) => (<div className={`calendar__day-event ${estado === 'PENDIENTE_CONFIRMACION' ? 'pending' : estado === 'CANCELADA' ? 'canceled' : estado === 'COMPLETADA' ? 'complete' : ''}`} key={i} ></div>))
                }
            </div>
        </div>
    )
